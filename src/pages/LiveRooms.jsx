...
export default function LiveRooms() {
  ...
  const getVirtualHome = (room) => {
    const hostProfile = profiles.find(p => p.display_name === room.host_name || p.username === room.host_name);
    return hostProfile?.cover_image || null;
  };
      ...
      {isLoading ? (
        ...
        <div className="p-4 space-y-3">
          {activeRooms.map(room => {
            const virtualHome = getVirtualHome(room);
            return (
